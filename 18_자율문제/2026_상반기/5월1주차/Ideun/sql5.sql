...
SELECT 
    TO_NUMBER(TO_CHAR(OS.SALES_DATE,'YYYY')) AS YEAR
    ...
    , ROUND(COUNT(DISTINCT(OS.USER_ID)) / (SELECT CNT FROM C),1) AS PURCHASED_RATIO
FROM USER_INFO UI
    JOIN ONLINE_SALE OS 
...
GROUP BY 
    TO_NUMBER(TO_CHAR(OS.SALES_DATE,'YYYY'))
